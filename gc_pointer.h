...
Pointer<T,size>::Pointer(const Pointer &ob){
    ...
    // TODO: Implement Pointer constructor
    // Lab: Smart Pointer Project Lab(it is ref lab)
    typename std::list<PtrDetails<T> >::iterator p;
...
T *Pointer<T, size>::operator=(T *t){
    ...
    // Here we are declaring a list of iterator of type PtrDetails
    typename std::list<PtrDetails<T> >::iterator y;
    y = findPtrInfo(t);
    // Increasing the reference count for the memory currently being pointed to
    y->refcount++;
    addr = t;
    // Here you can also use the following statement (commented out) since refContainerSize()
    // returns size 
    // arraySize = size;
...
Pointer<T, size> &Pointer<T, size>::operator=(Pointer &rv){
    ...
    // addr points to the allocated memory to which
    // this Pointer pointer currently points.
    // so we are getting the address of the pointer rv
    ...
    // Calling the destructor on the Pointer rv to delete the pointer.
    rv.~Pointer();
    return addr;
